...
const TruckMap = () => {
  ...
  const polylinesRef = useRef({}); // Add reference for polylines
          ...
          if (selectedTruck && selectedTruck.truck_id) {
            ...
            if (updatedSelectedTruck) {
              ...
              setSelectedTruck({
                ...
                route_points: updatedSelectedTruck.route?.route_points || [], // Add route points
                polyline_color: updatedSelectedTruck.route?.polyline_color || "#00008B", // Add polyline color
              });
            } else {
  ...
  const updateAvailableFilters = (data) => {
    ...
    // Date ranges based on scheduled collection
  ...
  const drawRoutePolyline = (record) => {
    ...
    try {
      // Convert route_points to google.maps.LatLng array
      const path = route.route_points.map(point =>
        new window.google.maps.LatLng(point.lat, point.lng)
      );

      // Create polyline with color from route data
  ...
  useEffect(() => {
    ...
    // Clear all existing markers and polylines first
    ...
    // Create markers and routes for all filtered records
      ...
      try {
        ...
        marker.addListener("click", () => {
          ...
          setSelectedTruck({
            ...
            route_points: record.route?.route_points || [], // Add route points
            polyline_color: record.route?.polyline_color || "#00008B", // Add polyline color
          });
        });
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors"
                >
                  {availableFilters.driverNames.map((driver) => (
              ...
              {/* <div>
                ...
                <select
                  ...
                  className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors"
                >
                  {availableFilters.scheduleStatuses.map((status) => (
                    <option key={status} value={status}>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <div>
                        ...
                        <span className="font-medium text-gray-800">
                          {record.recurring_day.charAt(0).toUpperCase() + record.recurring_day.slice(1)}
                        </span>
                      </div>
                    </div>
