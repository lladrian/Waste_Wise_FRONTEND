...
import { createRoute, getAllRoute, deleteRoute, updateRoute } from "../../hooks/route_hook";
...
const RouteManagementLayout = () => {
    ...
    // Handle route points from the map component
    const handleRouteChange = useCallback((updatedPoints) => {
        setRoutePoints(updatedPoints);
    }, []);

    // Handle color change from the map component
    ...
    useEffect(() => {
        if (formData.barangays && formData.barangays.length > 0) {
            ...
            if (firstBarangay && firstBarangay._id) {
                fetchBarangayPosition(firstBarangay._id);
            } else {
                setFirstBarangayPosition(null);
            }
        } else {
    ...
    const handleEdit = async (route) => {
        ...
        // Set route points from the backend data
        if (route.route_points && Array.isArray(route.route_points)) {
            setRoutePoints(route.route_points.map(point => ({
                lat: point.lat,
                lng: point.lng
            })));
        } else {
        ...
        // Set polyline color from backend data
        if (route.polyline_color) {
            setPolylineColor(route.polyline_color);
        } else {
            setPolylineColor('#009900');
        }
        ...
        if (route.merge_barangay && route.merge_barangay.length > 0) {
            barangaysArray = route.merge_barangay.map((barangay, index) => {
                if (typeof barangay === 'object') {
                    if (barangay.barangay_id) {
                        return {
                            _id: barangay.barangay_id._id || barangay.barangay_id,
            ...
            // Fetch position of first barangay
            if (barangaysArray.length > 0) {
                const sortedBarangays = [...barangaysArray].sort((a, b) => a.order_index - b.order_index);
                const firstBarangay = sortedBarangays[0];
                if (firstBarangay && firstBarangay._id) {
                    await fetchBarangayPosition(firstBarangay._id);
                }
            }
        }
    ...
    const getInitialLocation = () => {
        ...
        // Priority 2: First route point if available (for editing)
                                            ...
                                            <div className="mb-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
                                                ...
                                                <div className="space-y-2 max-h-40 overflow-y-auto">
                                                    {formData.barangays
                                                        ...
                                                        .map((barangay, index) => (
                                                            <div key={`${barangay._id}-${index}`} className="flex items-center justify-between bg-white p-3 rounded border">
                                    ...
                                    <div className="md:col-span-2">
                                        <div className="relative">
                                            ...
                                            <MapLocationMarkerDraw
                                                ...
                                                initCenter={firstBarangayPosition}
                                            />
                                        </div>
                                    </div>
                                </div>
