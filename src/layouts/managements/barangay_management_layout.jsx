...
import { getSpecificBarangay, createBarangay, getAllBarangay, getAllGarbageSiteSpecifcBarangay, deleteBarangay, updateBarangay, getAllBarangayList } from "../../hooks/barangay_hook";
...
const BarangayManagementLayout = () => {
    ...
    const fetchDataBarangayList = async () => {
        try {
            const { data, success } = await getAllBarangayList();
            if (success === true) {
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    {console.log(barangayList)}
                                            {barangayList.map((barangay, index) => (
                                                <option key={barangay + " " + index} value={barangay}>
                                                    {barangay}
                                                </option>
                                            ))}
                                        </select>
